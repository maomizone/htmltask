...
var imgUrl = str.getQuery("imgUrl");
setUrl(url,imgUrl);
// 维修美容的时候传appointID和state
...
// 是否是管理者
var isManager = str.getQuery("isManager");


// saleID="df4e5469-36d5-4ba1-942b-85ea2fc158be";
// isManager=0;
...
function repair_pay_info(vueObj) {
    ...
    $.ajax({
        url:RepairManage + "repair_pay_info",
        type:"POST",
        data: obj,
        timeout:1000*10,
        dataType:"json",
        ...
        success:function (data,textStatus,jqXHR) {
            ...
            if(data.status == 1){
                ...
                // 支付状态 （0:待支付,1:已支付）
                if(vueObj.payInfo.state == 0){
                    vueObj.stateStr = '待支付';
                }else if(vueObj.payInfo.state == 1){
                    vueObj.stateStr = '待评价';
                }
            }else {
...
function member_pay_info(vueObj) {
    ...
    $.ajax({
        url:MemberManage + "member_pay_info",
        type:"POST",
        data: obj,
        timeout:1000*10,
        dataType:"json",
