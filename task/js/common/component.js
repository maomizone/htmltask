...
Vue.component('project-template', {
    ...
    template: '<div style="background-color: #F8F8F8">'+
    // '<div style="padding:0px 12px;height:60px;box-sizing: border-box;display: flex;align-items: center">' +
    '<div style="padding:8px 12px;box-sizing: border-box;display: flex;align-items: center">' +
    '<span class="font font-28-666" style="flex: 1">{{projectName}}</span>' +
    '<div style="float: right; display: flex; flex-direction: column;align-items: flex-end;justify-content: center">' +
    '<span class="font font-28-orange">￥{{p1}}</span>' +
    '<del class="font font-26-999" v-bind:style="{ display: show }">￥{{p2}}</del>' +
    '<span class="font font-24-666">x{{parseFloat(project.projectNum)}}</span>' +
    '</div>' +
    '</div>' +
    '<div class="divider" style="margin-left: 12px"></div>' +
    '</div>'
...
Vue.component('product-template', {
    ...
    '<div style="flex: 1;display: flex; flex-direction: column;justify-content: center;">' +
    '<span class="font font-28-666">{{product.productName}}</span>' +
    ...
    '<span class="font font-28-orange">￥{{p1}}</span>' +
    '<del class="font font-26-999" v-bind:style="{ display: show }">￥{{p2}}</del>' +
    '<span class="font font-24-666">x{{parseFloat(product.productNum)}}</span>' +
    '</div>' +
    '</div>' +
...
Vue.component('charge-template', {
    ...
    '<div>' +
    '<span class="font font-28-999">总计</span>' +
    '<span class="font font-28-999" style="float: right">￥{{productMoneys}}</span>' +
    '</div>'+

    '<div>' +
    '<span class="font font-28-999">运费</span>' +
    '<span class="font font-28-999" style="float: right">+￥{{freightMoney}}</span>' +
    '</div>'+

    '<div>' +
    '<span class="font font-28-999">客户优惠</span>' +
    '<span class="font font-28-999" style="float: right">-￥{{discountMoney}}</span>' +
    '</div>'+

    '<div v-if="activityMoney!=0">' +
    '<span class="font font-28-999">活动优惠</span>' +
    '<span class="font font-28-999" style="float: right">-￥{{activityMoney}}</span>' +
    '</div>'+

    '<div v-if="specialMoney!=0">' +
    '<span class="font font-28-999">特批优惠</span>' +
    '<span class="font font-28-999" style="float: right">-￥{{specialMoney}}</span>' +
    '</div>'+

    '<div>' +
    '<span class="font font-28-333">订单总价</span>' +
    '<span class="font font-28-333" style="float: right">￥{{orderMoney}}</span>' +
    '</div>'+
    ...
    '<div style="height: 20px">' +
        '<strong class="font font-28-999" style="float: right">支付明细</strong>'+
    '</div>' +

    '<div v-if="redMoney!=0">' +
        '<span class="font font-28-999">红包</span>' +
        '<span class="font font-28-999" style="float: right">-￥{{redMoney}}</span>' +
    '</div>' +

    '<div v-if="isFromHistoryRepair == 1 && cardMoney!=0">' +
        '<span class="font font-28-999">卡券</span>' +
        '<span class="font font-28-999" style="float: right">-￥{{cardMoney}}</span>' +
    '</div>' +
    ...
    '<div>' +
        '<span class="font font-30-333">{{state == 0 ? "应付款":"实付款"}}</span>' +
        '<strong class="font font-30-orange" style="float: right">￥{{payMoney}}</strong>' +
    '</div>' +
...
Vue.component('charge-template-online', {
    props: {
        info:Object,
        state:Number,
    },
    data: function () {
        var productTotal = this.getMoney(this.info.productTotal);
        var discountMoney = this.getMoney(this.info.discountMoney);
        var orderTotal = this.getMoney(this.info.orderTotal);
        var cardMoney = this.getMoney(this.info.cardMoney);
        var realMoney = this.getMoney(this.info.realMoney);
        var usedCardList = this.info.usedCardList;
    ...
    '<div>' +
    '<span class="font font-28-999">总计</span>' +
    '<span class="font font-28-999" style="float: right">￥{{productTotal}}</span>' +
    '</div>'+

    '<div>' +
    '<span class="font font-28-999">运费</span>' +
    '<span class="font font-28-999" style="float: right">+￥0.00</span>' +
    '</div>'+
    ...
    '<div>' +
    '<span class="font font-28-333">订单总价</span>' +
    '<span class="font font-28-333" style="float: right">￥{{orderTotal}}</span>' +
    '</div>'+
    ...
    '<div v-if="cardMoney!=0">' +
    '<span class="font font-28-999">卡券</span>' +
    '<span class="font font-28-999" style="float: right">-￥{{cardMoney}}</span>' +
    '</div>' +

    '<div v-if="usedCardList && usedCardList.length > 0">' +
    '<template v-for="card in usedCardList">' +
    '<div>' +
    '<span class="font font-24-999">{{card.cardName}}</span>' +
    '<span class="font font-24-999" style="float: right">-￥{{parseFloat(card.cardMoney).toFixed(2)}}</span>' +
    '</div>'+
    '</template>' +
    '</div>' +
    ...
    '<div>' +
    '<span class="font font-30-333">{{state == 0 ? "应付款":"实付款"}}</span>' +
    '<strong class="font font-30-orange" style="float: right">￥{{realMoney}}</strong>' +
    '</div>' +
