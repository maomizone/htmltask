...
(function ($) {
    ...
    $.fn.mdater = function (config) {
        ...
        $.fn.changeData = function (data, selectedDate, year, month) {
            option.data = data;
            mdater.refreshView(selectedDate, year, month, false);
            if(!stringIsBlank(selectedDate)){
        ...
        $.fn.getYearMonth = function (dateStr) {
            console.log("djy getYearMonth dateStr " + dateStr);
            var date = new Date(Date.parse(dateStr.replace(/-/g, '/'))); //将日期值格式化

            var year = date.getFullYear();
            var month = F.getMonth(date.getMonth());
            return [year, month];
        };
        ...
        $.fn.setSelectedDate = function (selectedDate) {
            mdater.refreshSelectedDayView(selectedDate);
        ...
        var mdater = {
            ...
            _showPanel: function (container) {
                ...
                // 显示选中日期的面板
                this.refreshSelectedDayView(getSelectedDate());
            ...
            _changeMonth: function (add, checkDate) {
                ...
                this.updateDate(add);
                option.onChangeMonth(this.value.year, F.getMonth(num));
            },
            ...
            getDateStr: function (y, m, selectedDate) {
                var dayStr = '';
                ...
                for (var j = startDay; j <= endDay; j++) {
                    ...
                    if (y == date.getFullYear() && m == date.getMonth() && j == date.getDate()) {
                        current += ' current';
                    }
            ...
            updateDate: function (add) {
                ...
                newDateArea.html(this.getDateStr(this.value.year, this.value.month, getSelectedDate()));
            ...
            // year 显示的年
            // year 显示的月
            refreshView: function (selectedDate, year, month) {
                var y = this.value.year = year;
                var m = this.value.month = month - 1;

                $('.yeartag').text(y).data('year', y);
                $('.monthtag').text(y + "年" + F.getMonth(m) + '月').data('month', m);
                var dayStr = this.getDateStr(y, m, selectedDate);
            ...
            // 显示选中日期的面板
            // selectedDate 选中的日期
            refreshSelectedDayView: function (selectedDate) {
                var day = new Date(Date.parse(selectedDate.replace(/-/g, '/'))); //将日期值格式化
                var y = this.value.year = day.getFullYear();
                var m = this.value.month = day.getMonth();
            ...
            initListeners: function () {
                ...
                panel.delegates({
                    ...
                    '#now': function () {
                        // 显示今天的面板
                        _this.refreshSelectedDayView(new Date().format("yyyy-MM-dd"));
                        var date = new Date().format("yyyy-MM-dd");
                        option.onSelected(date, F.getWeekDay(date),  F.getAppointNum(date));
                        option.onChangeMonth(new Date().getFullYear(),  F.getMonth(new Date().getMonth()));
                        $('.md_datearea li').click(function () {
                        });
                    },
                    '.md_datearea li': function () {
                        ...
                        option.onSelected(_this.value.year + '.' + monthValue + '.' + dateValue,
                            F.getWeekDay(_this.value.year + '-' + monthValue + '-' + dateValue),
                            F.getAppointNum(_this.value.year + '-' + monthValue + '-' + dateValue))
                    },
