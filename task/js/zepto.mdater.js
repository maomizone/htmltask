...
(function ($) {
    ...
    $.fn.mdater = function (config) {
        ...
        $.fn.changeData = function (data, dateStr) {
            option.data = data;
            mdater.refreshView(dateStr);
            if(!stringIsBlank(dateStr)){
        ...
        $.fn.setSelectedDate = function (dateStr) {
            if(!stringIsBlank(dateStr)){
                var arr = dateStr.split('-');
                var date = new Date(arr[0], arr[1] - 1, arr[2]).format("yyyy-MM-dd");
                option.onSelected(date, F.getWeekDay(date), F.getAppointNum(date));
        ...
        var mdater = {
            ...
            _showPanel: function (container) {
                this.refreshView(geteSelectedDate());
            ...
            //每次调出panel前，对界面进行重置
            refreshView: function (initVal) {
                // var initVal = input.val(),
                var date = null;

                if (!stringIsBlank(initVal)) {
